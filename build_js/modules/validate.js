import RegExpRule from 'build_js/api/RegExpRule';
	...
	_self.config = function (formTarget, options) {
		...
		$.each(options, function (item) {
			(function (item) {
				...
				if (formElement.find('input[name=' + item + ']').length < 1 && formElement.find('select[name=' + item + ']').length < 1) {
					return;
				}
				...
				$childEle.on('change', function () {
					...
					if($(this).attr('id') == 'country'){
						if(!($('.add').css('display')=='block' && $('#newAddressForm input[name="telephone"]').val() == '' && $('#newAddressForm input[name="postcode"]').val()=='')){
				...
				$childEle.on('focus', function () {
					if($(this).attr('id') == "telephone" && ($(this).val().indexOf('*') > 0)){
						$(this).val('');
					}
...
formValid.prototype = {
	...
	rules: {
		...
		password: function (element, errorMsg) {
			...
			if(formId.selector == '#unionLogin' || formId.selector == '#loginForm'){
				pwd = RegExpRule.loginPwdRule;
				errorMsg = '4-30个数字、字母或符号区分大小写';
			}
			if (value === '') {
				return;
		...
		captcha: function (element, errorMsg,isCheck) {
			...
			if (value === '') {
				...
				if(isCheck){
					check = true;
					// serviceData.checkCaptcha(element); //检测验证码是否合法,待优化
		...
		phoneCaptcha: function (element, errorMsg) {
			var value = element.val(),
				self = this;
			if(formId.selector.includes('email')){
				return true;
			}else if (value === '') {
				return;
		...
		emailCaptcha: function (element, errorMsg) {
			var value = element.val(),
				self = this;
			if(formId.selector.includes('cellphone') || formId.selector.includes('Phone')){
				return true;
			}else if (value === '') {
				return;
		...
		compared: function (element, compareId, errorMsg) {
			...
			if (value === '') {
				return;
			} else if (formId.selector == '#changePasswordForm' && (formId.find('input[name="current_password"]').val() == formId.find('input[name="password"]').val())) {
				$(element).attr('valid', false);
		...
		address: function (element, errorMsg) {
			...
			if (value === '') {
				return;
			} else if (!RegExpRule.addressRule.test(value) && (value == '国家、地域' || value == '省份' || value == '地级市' || value == '市、县级市')) {
				$(element).attr('valid', false);
				return self.process.errorHold.call(self, element, errorMsg);
			}else{
				$(element).attr('valid', true);
	...
	process: {
		...
		errorHold: function (element, msg) {
			...
			item.find('.accout-error').addClass('form-error').html('<i class="icon-error"></i>' + msg).addClass("active").attr('sel-id', 'error-tip');
