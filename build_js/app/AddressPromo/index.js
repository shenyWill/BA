...
var addressPromoHbs = require('build_js/hbs/addressPromo.hbs');
...
var addressFn = function () {
    ...
    this.initAddress = function(){
        ...
        serviceData.requestdata(url,{
            ...
            if(data.status == 200){
                ...
                var htmlDom = addressPromoHbs(dataObj);
    ...
    this.initCancelAddNewAddressHandler = function () {
        $('.btn-cancel').on('click', function (event) {
            ...
            _checkCountry(-1);
            _checkGegion(-1);
            $('#s_county').html('<option value="市、县级市">市、县级市</option>');
        });
    }
    ...
    function _extraAddressValidate() {
        ...
        console.log(postcode + ":" + country + ":" + region_id + ":" + city + ":" + s_county);
        ...
        if ($("#country").val() == "中国大陆" && (postcode == "000000" || postcode.length != 6) ) {
            ...
            $("#postcode").siblings('.error-tips').addClass("form-error").addClass("active").html('<i class="icon-error"></i>邮政编码错误');
            return false;
        } else {
            return true;
        }
    ...
    function _getAddressParams() { //获取新增/修改地址表单数据
        ...
        addressParams["default_shipping"] = $newAddressForm.find("#clauseCheckbox").is(":checked") ? 1 : 0;
